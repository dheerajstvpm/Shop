...
const userLoginGet = (req, res) => {
  ...
  if (session.userStatus === "blocked") {
    ...
    res.render("users/login", {
      title: "Shop.",
      passwordMessage: "Username has been blocked",
    });
...
const userSignupGet = (req, res) => {
  ...
  if (session.userId) {
    ...
    res.render("users/signup", {
      title: "Shop.",
      mobileMsg: "User with this mobile number already exists",
    });
  } else if (session.signupErrors) {
    session.signupErrors = false;
    const error1 =
      signupErrors.errors.find((item) => item.param === "name") || "";
    const error2 =
      signupErrors.errors.find((item) => item.param === "mobileNumber") || "";
    const error3 =
      signupErrors.errors.find((item) => item.param === "username") || "";
    const error4 =
      signupErrors.errors.find((item) => item.param === "password") || "";
    const error5 =
      signupErrors.errors.find((item) => item.param === "confirmPassword") ||
      "";

    res.render("users/signup", {
      title: "Shop.",
      nameMsg: error1.msg,
      mobileMsg: error2.msg,
      usernameMsg: error3.msg,
      pwdMsg: error4.msg,
      confirmPwdMsg: error5.msg,
    });
  } else {
            ...
            for (const check of checks) {
              if (check._id == productId) {
                ...
                User.findOneAndUpdate(
                  { name: session.userId, "cart._id": productId },
                  { $inc: { "cart.$.count": 1 } },
                )
                  .then((result) => {
                    session.cartCount = result.cart.length;
              ...
              if (n > 0) {
                ...
                try {
                  ...
                  for (let cart of carts) {
                    cart = cart.toJSON();
                    try {
                      const out = await Offer.findOne({
                        offerName: cart.offer,
                      });
                      console.log(out.discount);
                      const discount = (out.discount * cart.price) / 100;
                      const offerPrice = cart.price - discount;
                  ...
                  const sum = function (items, prop1, prop2) {
                    return items.reduce(function (a, b) {
                      return (
                        parseInt(a) + parseInt(b[prop1]) * parseInt(b[prop2])
                      );
                    }, 0);
                  };
                  const total = sum(cartArray, "offerPrice", "count");
          ...
          .then((out) => {
            ...
            for (const check of checks) {
              if (check._id == productId) {
                ...
                check.count = check.count + 1;
                User.findOneAndUpdate(
                  { name: session.userId, "wishlist._id": productId },
      ...
      .then((result) => {
        ...
        Product.find({}).then((result) => {
          ...
          function operation() {
            ...
            // console.log(`l:${l}`)
            if (l === m * n) {
              User.findOne({ _id: session.uid }).then((result) => {
                session.cartCount = result.cart.length;
                session.wishlistCount = result.wishlist.length;
                  ...
                  async function f2() {
                    ...
                    try {
                      ...
                      const sum = function (items, prop1, prop2) {
                        return items.reduce(function (a, b) {
                          return (
                            parseInt(a) +
                            parseInt(b[prop1]) * parseInt(b[prop2])
                          );
                        }, 0);
                      };
                    ...
                    if (session.noValidOption) {
                      ...
                      res.render("users/buyNow", {
                        ...
                        total: total,
                        beforeTotal: beforeTotal,
                        msg: "Some items in your cart went out of stock",
                      });
                    } else if (session.addAddressError) {
                      session.addAddressError = false;
                      const error1 =
                        otpLoginErrors.errors.find(
                          (item) => item.param === "newAddress",
                        ) || "";
                      ...
                      res.render("users/buyNow", {
                        ...
                        total: total,
                        beforeTotal: beforeTotal,
                        msg: "Enter key not allowed in address field.",
                      });
            ...
            for (const cartItem of cartItems) {
              for (const product of result) {
                ...
                if (x == y && cartItem.count > product.stock) {
                  ...
                  try {
                    const result = await User.findOneAndUpdate(
                      { name: session.userId, "cart._id": product._id },
                      { $set: { "cart.$.count": product.stock } },
                    );
                    session.cartCount = result.cart.length;
                    session.wishlistCount = result.wishlist.length;
  ...
  if (session.userId) {
    ...
    if (!otpLoginErrors.isEmpty()) {
      ...
      if (session.addressExistErr != true && req.body.newAddress != "") {
        ...
        await User.findOneAndUpdate(
          { _id: session.uid },
          { $push: { address: req.body.newAddress } },
        );
      }
      session.addressExistErr = false;
  ...
  if (session.userId) {
    ...
    for (let order of orders) {
      ...
      if (order.unique === uniqueId) {
        await User.findOneAndUpdate(
          { name: session.userId, "order.unique": uniqueId },
          {
            $set: {
...
const userMobileLoginGet = (req, res) => {
  ...
  if (session.userId) {
    ...
    const error1 =
      otpLoginErrors.errors.find((item) => item.param === "mobile") || "";
    ...
    res.render("users/mobileLogin", {
      title: "Shop.",
      mobileMsg: "Mobile number not found",
    });
  } else {
    ...
    User.findOne({ mobile: req.body.mobile }).then((result) => {
      if (result) {
        client.verify
          ...
          .then((data) => {
            (mobileNumber2 = req.body.mobile), res.redirect("/otpLoginVerify");
          });
      } else {
        console.log(result);
        session = req.session;
    ...
    User.findOne({ _id: session.uid })
      .then((result) => {
        if (session.mobileAlreadyExist) {
          ...
          const error1 =
            otpLoginErrors.errors.find((item) => item.param === "newName") ||
            "";
          ...
          const error1 =
            otpLoginErrors.errors.find((item) => item.param === "newMobile") ||
            "";
          console.log(otpLoginErrors);
          res.render("users/profile", {
            title: "Shop.",
          ...
          session.addAddressError = false;
          const error1 =
            otpLoginErrors.errors.find((item) => item.param === "newAddress") ||
            "";
          console.log(otpLoginErrors);
          res.render("users/profile", {
            title: "Shop.",
...
const nameEdit = (req, res) => {
  ...
  if (!otpLoginErrors.isEmpty()) {
    ...
    User.findOneAndUpdate(
      ...
      .then(() => {
        (session.userId = req.body.newName), res.redirect("/profile");
      })
      .catch((err) => {
        console.log(err);
      });
...
const mobileEdit = (req, res) => {
  ...
  if (!otpLoginErrors.isEmpty()) {
    ...
    User.findOne({ mobile: req.body.newMobile }).then((result) => {
      console.log(result);
      if (result !== null) {
        session = req.session;
        session.mobileAlreadyExist = true;
        ...
        client.verify
          ...
          .then((data) => {
            (mobileNumber3 = req.body.newMobile),
              res.render("users/mobileChangeOtp", {
                title: "Shop.",
                loginName: session.userId,
              });
          });
...
const changePasswordGet = (req, res) => {
  ...
  if (session.changePwdErrors) {
    ...
    const error4 =
      changePwdErrors.errors.find((item) => item.param === "password") || "";
    const error5 =
      changePwdErrors.errors.find((item) => item.param === "confirmPassword") ||
      "";
...
const saveOrder = async function (req, res) {
  ...
  for (let cartItem of cartItems) {
    ...
    await Product.updateOne(
      { _id: stockId },
      { $inc: { stock: removeCount, sales: salesCount } },
    );

    // Update coupon
    await Coupon.updateOne(
      { coupon: session.coupon },
      { $push: { users: session.uid } },
    );
...
async function generateAccessToken() {
  const auth = Buffer.from(
    process.env.paypalClientid + ":" + process.env.paypalClientsecret,
  ).toString("base64");
