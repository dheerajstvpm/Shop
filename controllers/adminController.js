...
const adminLoginPost = function (req, res) {
  ...
  Admin.find({ username: req.body.username })
    .then((result) => {
      ...
      bcrypt
        .compare(req.body.password, temp.password)
...
const adminProductSearch = function (req, res) {
  ...
  if (adminSession.adminId) {
    Product.find({
      $or: [{ productName: req.body.input }, { category: req.body.input }],
    })
      .then((result) => {
  ...
  if (!errors.isEmpty()) {
    ...
    try {
      ...
      res.render("admin/addNewProduct", {
        ...
        offerResult,
        categoryResult,
        productNameMsg: error1.msg,
        descriptionMsg: error2.msg,
        priceMsg: error3.msg,
        stockMsg: error4.msg,
        image1Msg: image1Msg,
        image2Msg: image2Msg,
        image3Msg: image3Msg,
        categoryMsg: error5.msg,
        admin: true,
      });
    } catch (err) {
      console.log(err);
          ...
          for (let cart of carts) {
            ...
            if (cartId === newProductId) {
              result2 = await User.updateOne(
                { _id: result._id, "cart._id": newProductId },
                { $set: { "cart.$.description": req.body.newDescription } },
              );
            }
          }
        }
        ...
        for (result of results) {
          ...
          for (let wishlist of wishlists) {
            ...
            if (wishlistId === newProductId) {
              result2 = await User.updateOne(
                { _id: result._id, "wishlist._id": newProductId },
                { $set: { "wishlist.$.description": req.body.newDescription } },
              );
            }
          }
        }
        ...
        for (result of results) {
          ...
          for (let cart of carts) {
            ...
            if (cartId === newProductId) {
              result2 = await User.updateOne(
                { _id: result._id, "cart._id": newProductId },
                { $set: { "cart.$.price": req.body.newPrice } },
              );
            }
          }
        }
        ...
        for (result of results) {
          ...
          for (let wishlist of wishlists) {
            ...
            if (wishlistId === newProductId) {
              result2 = await User.updateOne(
                { _id: result._id, "wishlist._id": newProductId },
                { $set: { "wishlist.$.price": req.body.newPrice } },
              );
            }
          }
        }
        ...
        for (result of results) {
          ...
          for (let cart of carts) {
            ...
            if (cartId === newProductId) {
              result2 = await User.updateOne(
                { _id: result._id, "cart._id": newProductId },
                { $set: { "cart.$.category": req.body.newCategory } },
              );
            }
          }
        }
        ...
        for (result of results) {
          ...
          for (let cart of carts) {
            ...
            if (cartId === newProductId) {
              result2 = await User.updateOne(
                { _id: result._id, "cart._id": newProductId },
                { $set: { "cart.$.offer": req.body.newOffer } },
              );
            }
          }
        }
...
const adminUserSearch = function (req, res) {
  ...
  if (adminSession.adminId) {
    Product.find({
      $or: [{ name: req.body.input }, { username: req.body.input }],
    })
      .then((result) => {
        if (adminSession.adminId && req.body.input) {
          res.render("admin/adminUserManagement", {
...
const adminCategoryManagement = async function (req, res) {
  ...
  if (adminSession.categoryProductExist) {
    ...
    res.render("admin/adminCategoryManagement", {
      ...
      newArray,
      categoryMsg:
        "Can not delete this category. Product in this category exists.",
      admin: true,
    });
  } else if (adminSession.adminId) {
  ...
  if (adminSession.adminId) {
    if (adminSession.categoryExist) {
      ...
      res.render("admin/addNewCategory", {
        title: "Shop.admin",
        categoryMsg: "Category already exist",
        admin: true,
      });
    } else {
...
const adminOfferManagement = function (req, res) {
  ...
  Offer.find({})
    ...
    .then((result) => {
      if (adminSession.adminId) {
        res.render("admin/adminOfferManagement", {
          title: "Shop.admin",
          result,
          admin: true,
        });
...
const addNewOfferPost = function (req, res) {
  ...
  if (!errors.isEmpty()) {
    res.render("admin/addNewOffer", {
      title: "Shop.admin",
      minOrderMsg: error1.msg,
      discountMsg: error2.msg,
      maxDiscountMsg: error3.msg,
      offerMsg: error4.msg,
      admin: true,
    });
  } else if (adminSession.adminId) {
  ...
  if (adminSession.adminId) {
    Offer.findOne({ _id: offerId })
      .then((result) => {
        ...
        // current.admin = true;
        console.log(result);
        res.render("admin/adminEditOffer", {
          title: "Shop.admin",
          result,
          admin: true,
        });
  ...
  if (adminSession.adminId) {
    if (req.body.newName) {
      try {
        const result = await Offer.findOneAndUpdate(
          { _id: newOfferId },
          { $set: { offerName: req.body.newName } },
        );
        const prdt = await Product.updateMany(
          { offer: result.offerName },
          { $set: { offer: req.body.newName } },
        );
      } catch (err) {
        console.log(err);
      }
    ...
    if (req.body.newMinOrder) {
      try {
        const result = await Offer.updateOne(
          { _id: newOfferId },
          { $set: { minOrder: req.body.newMinOrder } },
        );
      } catch (err) {
        console.log(err);
      }
    ...
    if (req.body.newDiscount) {
      try {
        const result = await Offer.updateOne(
          { _id: newOfferId },
          { $set: { discount: req.body.newDiscount } },
        );
      } catch (err) {
        console.log(err);
      }
    ...
    if (req.body.newMaxDiscount) {
      try {
        const result = await Offer.updateOne(
          { _id: newOfferId },
          { $set: { maxDiscount: req.body.newMaxDiscount } },
        );
      } catch (err) {
        console.log(err);
      }
  ...
  if (adminSession.adminId) {
    Product.find({})
      .then((result) => {
        ...
        res.render("admin/userHomepageLayout", {
          title: "Shop.admin",
          result,
          admin: true,
        });
...
const adminAllOrderManagementGet = async (req, res) => {
  ...
  if (adminSession.adminId) {
    ...
    res.render("admin/adminAllOrderManagement", {
      title: "Shop.admin",
      admin: true,
      order,
      page,
...
const adminDatatableOrderManagementGet = async (req, res) => {
  ...
  if (adminSession.adminId) {
    ...
    res.render("admin/adminDatatableOrderManagement", {
      title: "Shop.admin",
      admin: true,
      orders,
    });
    ...
    for (let order of orders) {
      ...
      if (order.unique === uniqueId) {
        await User.updateOne(
          { _id: customerId, "order.unique": uniqueId },
          { $set: { "order.$.orderStatus": "Order cancelled" } },
        );
        await User.updateOne(
          { _id: customerId, "order.unique": uniqueId },
          { $set: { "order.$.cancelBtn": false } },
        );
        await User.updateOne(
          { _id: customerId, "order.unique": uniqueId },
          { $set: { "order.$.returnBtn": false } },
        );
        await User.updateOne(
          { _id: customerId, "order.unique": uniqueId },
          { $set: { "order.$.updateBtn": false } },
        );
        await Product.updateOne(
          { _id: order._id },
          { $inc: { stock: order.count, sales: order.count * -1 } },
        );
      }
    }
    res.redirect("back");
      ...
      for (let order of orders) {
        ...
        if (order.unique === uniqueId) {
          ...
          if (req.body.status === "Cancelled") {
            ...
            await User.updateOne(
              { "order.unique": uniqueId },
              {
                $set: {
                  "order.$.orderStatus": "Order cancelled",
                  "order.$.cancelBtn": false,
                  "order.$.returnBtn": false,
                  "order.$.updateBtn": false,
                },
              },
            );
            await Product.updateOne(
          ...
          for (result of results) {
            ...
            for (let cart of carts) {
              ...
              if (cartCategory === req.params.category) {
                result2 = await User.updateOne(
                  { _id: result._id, "cart._id": cart._id },
                  { $set: { "cart.$.offer": req.body.offer } },
                );
              }
            }
          }
...
const adminCouponManagement = async function (req, res) {
  ...
  Coupon.find({})
    ...
    .then((result) => {
      if (adminSession.adminId) {
        res.render("admin/adminCouponManagement", {
          title: "Shop.admin",
          result,
          admin: true,
        });
...
const addNewCouponPost = async function (req, res) {
  ...
  if (adminSession.adminId) {
    ...
    const error2 =
      errors.errors.find((item) => item.param === "reduction") || "";
    const error3 =
      errors.errors.find((item) => item.param === "minOrder") || "";
    if (!errors.isEmpty()) {
      res.render("admin/addNewCoupon", {
        title: "Shop.admin",
        couponMsg: error1.msg,
        reductionMsg: error2.msg,
        minOrderMsg: error3.msg,
        admin: true,
      });
    } else {
...
const adminEditCouponGet = async function (req, res) {
  ...
  if (adminSession.adminId) {
    ...
    console.log(result);
    res.render("admin/adminEditCoupon", {
      title: "Shop.admin",
      result,
      admin: true,
    });
  ...
  if (adminSession.adminId) {
    if (req.body.newName) {
      await Coupon.updateOne(
        { _id: req.params.id },
        { $set: { coupon: req.body.newName } },
      );
    }
    if (req.body.newReduction) {
      await Coupon.updateOne(
        { _id: req.params.id },
        { $set: { reduction: req.body.newReduction } },
      );
    }
    if (req.body.newMinOrder) {
      await Coupon.updateOne(
